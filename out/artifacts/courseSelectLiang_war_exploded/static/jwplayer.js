...
(function(jwplayer) {
    ...
    var embed = jwplayer.embed = function(playerApi) {
        ...
        function _doEmbed() {
            ...
            if (_pluginloader.getStatus() === utils.loaderstatus.COMPLETE) {
                ...
                var message;
   	            var errormsg='<img src="/static/lib/jwplayer/1.0.0/flasherror.png" width="380" height="280" usemap="#Map" border="0" /><map name="Map" id="Map"><area shape="rect" coords="215,240,374,278" href="http://www.imooc.com/user/feedback" target="_blank" /><area shape="rect" coords="6,239,165,277" href="http://get.adobe.com/cn/flashplayer/" target="_blank" /></map>'
	
                if (_config.fallback) {
                    //message = 'No suitable players found and fallback enabled';
		            message =_config.fallbackMessage||errormsg;
	                _setupErrorTimer = setTimeout(function() {
                        _dispatchSetupError(message, true);
	                }, 10);
                    utils.log(message);
		            _sourceError({message:message});//display custom error message.below is the origin code.
    ...
    function _displayError(container, message, config) {
        ...
        //text.innerHTML='Why?? Failed to load!'+'<br>'+'Try to refresh or change a browser!'+'<br>'+'Still unable to watch? Please submit comments to feedback';
        container.innerHTML = '';
